...
The release workflow builds `sealg` per platform; the produced binaries can't
...
The optional frontend talks to `sealg serve` over `fetch`; the dev proxy and
live wiring are best verified in a browser.

- [ ] **Connectivity dot** - Run `sealg serve`, then `make dev`. Load the app:
...
## Onboarding prune (`sealg init`)

- [ ] **Pruned project builds** - In a scratch copy, run `sealg init` for each
